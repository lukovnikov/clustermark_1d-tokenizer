...
def distort_images(images: typing.Union[Image.Image, typing.List[Image.Image]],
    ...
    for img in images:
        ...
        if not isinstance(img, Image.Image) and (r_degree is not None or jpeg_ratio is not None or
        ...
        if sp_prob_fixed is not None:
            ...
            image = torch.where(mask < (sp_prob_fixed)/2, torch.zeros_like(image), image)
            image = torch.where(mask > 1 - (sp_prob_fixed)/2, torch.ones_like(image), image)
            img = transforms.ToPILImage()(image.clamp(0, 1))  # Converts back to PIL Image
...
def visualize(imgpath="experiments_v2.2/gen_wm_pp_v2.2_50000samples_GPT-L_c2i_wm_clusters64_penalty5_greenfrac=0.5/images/8.png", 
              distortions="brightness_factor=3,contrast_factor=1.5,hue_factor=0.1,saturation_factor=2",
              output="test.png",
                   ):
    """
    ...
    make_image_grid([img]+distorted_imgs, len(distorted_imgs), padding=2).save(output)
