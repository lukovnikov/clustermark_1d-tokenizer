import os
# Disable IPython integration in Fire to avoid compatibility issues
os.environ['FIRE_DISABLE_IPYTHON'] = '1'
...
from distortions_v2 import distort_images
from sample_c2i_wm import load_model
import time
...
class ClusterClassifier(nn.Module):
    ...
    def __init__(self, vqencoder, mapping=None, dim=256):
        """
        Args:
    ...
    def forward(self, x, tokens=None):   # VQ Encoder expects range between 0 and 1 
...
def train(
    ...
    t = time.time()
    print("loading model...")
    vq_model = load_model(modelsize=modelsize, onlyvae=True, device=device)
    t = time.time() - t
